...
var jogoArea = {
	...
	iniciar : function() {
		...
		if(!this.iniciou)
		{
			...
			this.contexto.canvas.width = 820;
			this.contexto.canvas.height = 601;
			//this.canvas.style.width = window.innerWidth;// + '; height: ' + 820/601 * window.innerHeight + ';';
		...
		if("ontouchstart" in document.documentElement)
		{
			...
			this.leftButton.construir(15, jogoArea.canvas.height/2*1.69, "sprites/leftButton.png"); // jogoArea.canvas.height/2*1.69
			this.rightButton = new MobileButton();
			this.rightButton.construir(130, jogoArea.canvas.height/2*1.69, "sprites/rightButton.png"); // jogoArea.canvas.height/2*1.69
			this.fireButton = new MobileButton();
			this.fireButton.construir(690, jogoArea.canvas.height/2*1.63, "sprites/actionButton.png"); // jogoArea.canvas.height/2*1.64
		...
		{
			if(window.innerHeight <= jogoArea.canvas.height*1.5)
			{
				jogoArea.canvas.style.height = window.innerHeight + 'px';
				jogoArea.canvas.style.width = window.innerHeight * 1.5 + 'px';
			}
			else
			{
				jogoArea.canvas.style.width = '100%';
				jogoArea.canvas.style.height = '100%';
			}
		}
		...
		function touchHandler(e)
		{
			...
			var rect = jogoArea.canvas.getBoundingClientRect(),
			scaleX = jogoArea.canvas.width / rect.width,
			scaleY = jogoArea.canvas.height / rect.height;
			jogoArea.touchX = (e.touches[0].clientX - rect.left) * scaleX;
			jogoArea.touchY = (e.touches[0].clientY - rect.top) * scaleY;
