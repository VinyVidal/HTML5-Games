...
function atualizarJogoArea() { // intervalo de atualizar a área
	...
	if(gameOver) {
		...
		gameOverText.construir(jogoArea.width/2/2, jogoArea.height/2, "#C22", null, 3.0, "50px OCR A Extended", "GAME OVER");
		gameOverText.escrever();
		if(atualScore.getScore() > highScore.getScore()) {
			highScore.setString("HI " + atualScore.getScore());
		}
		jogoArea.pausar();
		// setTimeout(function() {
			// jogoArea.iniciar();
